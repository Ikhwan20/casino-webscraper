...
0 11 * * * root cd /app && /usr/bin/python3 casino_scraper.py >> /app/logs/scraper_$(date +\%Y\%m\%d_\%H\%M\%S).log 2>&1 && sleep 10 && /usr/bin/python3 /app/run_processor.py >> /app/logs/processor_$(date +\%Y\%m\%d_\%H\%M\%S).log 2>&1
